...
<script type="text/javascript">
	...
	function loadDataGroupUser()
	{
	    ...
	    var dataTable = $('#IDTblGroupUser').DataTable( {
	        ...
	        "ordering" : false,
	        "ajax":{
	            url : base_url_js+"it/config/getAuthDataTables", // json datasource
	            ordering : false,
	...
	$(document).on('click','.btn-delete-groupauth', function () {
	  ...
	  $("#confirmYes").click(function(){
	      ...
	      var url =base_url_js+'it/config/authUser/cud';
	...
	function load_data_GroupPrevileges()
	{
		...
		$('#loadtableGroupPrevileges').html(HTMLTblGroupPrevileges);
		    var url = base_url_js+'it/menu/group_previleges/crud'
			var data = {
				    action : 'read',
			};
		var token = jwt_encode(data,'UAP)(*');
	...
	{
	    var url = base_url_js+"it/menu/save_menu";
	...
	function loadSubMenu_to_group()
	{
	    ...
	    var url = base_url_js+"it/menu/group_previleges/get_submenu_by_menu";
	    var data = {
	                Menu : value,
	                GroupUser : GroupUser
	                };
	...
	$(document).on('click','#btn-sbmt-user', function () {
		if (confirm('Are you sure ?')) {
			...
			if (getData.length > 0) {
				...
				var url = base_url_js+'it/menu/group_previleges/save_submenu_by_menu';
	...
	function loadDatamenuPrevilegesGroupUser()
	{
	    ...
	    var url = base_url_js+'it/menu/group_previleges/rud';
