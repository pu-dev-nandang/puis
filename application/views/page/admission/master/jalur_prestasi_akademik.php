    ...
    <div class="col-md-8">
        <div class="widget box">
            <div class="widget-header">
                <h4 class="header"><i class="icon-reorder"></i>Jalur Prestasi Akademik</h4>
...
<script type="text/javascript">
  ...
  $(document).on('click','.btn-add', function () {
     modal_generate('add','Add Potongan SPP');
  });

  $(document).on('click','.btn-edit', function () {
    var ID = $(this).attr('data-smt');
     modal_generate('edit','Edit Potongan SPP',ID);
  });
  ...
  $(document).on('click','#confirmYesDelete',function () {
        ...
        var url = base_url_js+'admission/master-registration/submit_jpa';
        var aksi = "delete";
        var ID = $(this).attr('data-smt');
        var data = {
            Action : aksi,
  ...
  function modal_generate(action,title,ID='') {
      var url = base_url_js+"admission/master-registration/modalform_jpa";
      var data = {
          Action : action,
          CDID : ID,
      };
  ...
  function loadTableJPA(callback)
  {
      ...
      '<thead>'+
          '<tr>'+
              ...
              '<th style="width: 106px;">Rangking Umum</th>'+
  ...
  function loadDataJPA()
  {
      var url = base_url_js+'admission/master-registration/jpa/table_jpa';
      ...
      $.post(url,function (data_json) {
          ...
          for (var i = 0; i < response.length; i++) {
              ...
              $("#Tbldata1 tbody").append(
                  '<tr>'+
                      '<td>'+no+'</td>'+
                      '<td>'+response[i]['StartRange']+' s/d '+response[i]['EndRange']+'</td>'+
