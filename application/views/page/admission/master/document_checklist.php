    ...
    <div class="col-md-8">
        <div class="widget box">
            <div class="widget-header">
                <h4 class="header"><i class="icon-reorder"></i>Document Checklist</h4>
                <div class="toolbar no-padding">
                    <div class="btn-group">
                      <span data-smt="" class="btn btn-xs btn-add">
                        <i class="icon-plus"></i> Add Document Checklist
                       </span>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
  ...
  $(document).on('click','.btn-add', function () {
     modal_generate('add','Add Document Checklist');
  });

  $(document).on('click','.btn-edit', function () {
    var ID = $(this).attr('data-smt');
     modal_generate('edit','Edit Document Checklist',ID);
  });
  ...
  function modal_generate(action,title,ID='') {
      var url = base_url_js+"admission/master-config/modalform/reg_doc_checklist";
      var data = {
          Action : action,
          CDID : ID,
      };
  ...
  function loadTable()
  {
    ...
    var url = base_url_js+'admission/master-config/loadTableMaster/reg_doc_checklist';
