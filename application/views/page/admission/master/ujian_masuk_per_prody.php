    ...
    <div class="col-md-8">
        <div class="widget box">
            <div class="widget-header">
                <h4 class="header"><i class="icon-reorder"></i>Set Ujian Masuk Per Prody</h4>
                <div class="toolbar no-padding">
                    <div class="btn-group">
                      <span data-smt="" class="btn btn-xs btn-add">
                        <i class="icon-plus"></i> Add Ujian Masuk Per Prody
                       </span>
                    </div>
                </div>
            </div>
...
<script type="text/javascript">
  ...
  $(document).on('click','#confirmYesDelete',function () {
        ...
        var url = base_url_js+'admission/master-registration/ujian-masuk-per-prody/submit';
        var aksi = "delete";
        var ID = $(this).attr('data-smt');
        var data = {
            Action : aksi,
  ...
  $(document).on('click','#ModalbtnSaveForm', function () {
    loading_button('#ModalbtnSaveForm');
     ...
     var nm_ujian = $("#nm_ujian").val();
     var selectBobot = $("#selectBobot").val();
     var selectPrody = $("#selectPrody").val();
  ...
  function modal_generate(action,title,ID='') {
      var url = base_url_js+"admission/master-registration/ujian-masuk-per-prody/modalform";
      var data = {
          Action : action,
          CDID : ID,
      };
  ...
  function loadTable()
  {
    ...
    var url = base_url_js+'admission/master-registration/ujian-masuk-per-prody/loadTable';
