					...
					<div class="col-md-12">
						<div class="col-xs-6 col-md-offset-3">
							<div class="thumbnail" style="height: 120px">
								<div class="row">
									<div class="col-md-12" align="center">
										<h4>Import Pengembalian Formulir</h4>
									</div>
								</div>
								<div class="row" style="margin-top: 10px">
									<div class="col-md-12">
										<div class="col-xs-6 col-md-offset-3" style="">
											<div class="col-xs-6">
												<label class="control-label">Upload File:</label>
												<input type="file" data-style="fileinput" id="ExFile3">
											</div>
											<div class="col-xs-6">
												<div class="col-xs-2">
													<button class="btn btn-inverse btn-notification" id="btn-proses3">Proses</button>
												</div>
											</div>
										</div>
										<a href="<?php echo base_url('download_template/admisi-rekap_pengembalian_formulir.xlsx'); ?>">File Template</a>
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	function processFile()
	{
		...
		$(document).on('click','#btn-proses3', function () {
			loading_button('#btn-proses3');
		  if (!window.File || !window.FileReader || !window.FileList || !window.Blob) {
		  		toastr.error('The File APIs are not fully supported in this browser.', 'Failed!!');
		        return;
		      }   

		      input = document.getElementById('ExFile3');
		      if (!input) {
		        toastr.error('Um, couldnot find the fileinput element.', 'Failed!!');
		        $('#btn-proses3').prop('disabled',false).html('Proses'); 
		      }
		      else if (!input.files) {
		        toastr.error('This browser doesnot seem to support the `files', 'Failed!!');
		        $('#btn-proses3').prop('disabled',false).html('Proses'); 
		...
		{
			var form_data = new FormData();
			var fileData = document.getElementById("ExFile3").files[0];
			var url = base_url_js + "admission/distribusi-formulir/offline/submit_import_excel_pengembalian_formulir_offline";
			form_data.append('fileData',fileData);
		  	$.ajax({
		  	  type:"POST",
		  	  url:url,
