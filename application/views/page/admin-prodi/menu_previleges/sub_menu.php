...
<script type="text/javascript">
	...
	var HTMLTbl = '<div class = "table-responsive">'+'<table class="table table-striped table-bordered table-hover table-checkable" id = "example_budget">'+
	             			'<thead>'+
	             				'<tr>'+
	...
	$(document).ready(function () {
       LoadData(Arr_dt);
       $('.btn_add_sub_menu').addClass(hideClass);
	});
	...
	function LoadData(dt)
	{
		...
		{
			var h = '';
			h = '<select class = " form-control actionCh '+hideClass+'" style = "width : 80%" '+disAccess+'>';
				var temp = ['Read','Write'];
		...
		var table = $('#example_budget').DataTable({
		      ...
		      'columnDefs': [
			      ...
			      },
			      {
			         'targets': 3,
			         'render': function (data, type, full, meta){
			             return '<input type="text" class="form-control Slug '+hideClass+'" value="'+full.Slug+'" '+disAccess+'>'+
			      ...
			      },
			      {
			         'targets': 4,
			         'render': function (data, type, full, meta){
			             return '<input type="text" class="form-control Controller '+hideClass+'" value="'+full.Controller+'" '+disAccess+'>'+
	...
	$(document).off('click', '.btn-save').on('click', '.btn-save',function(e) {
		...
		if (confirm('Are you sure ?')) {
		   ...
		   var url = base_url_js+"prodi/menu/sub_menu/save";
		   var data = {
			    action : action,
			    ID : ID,
			    ID_Menu : ID_Menu,
	  ...
	  for(var key in arr) {
	     switch(key)
	     {
	      ...
	      case  "Slug" :
	            ...
	            {
	            	if (aa.length != 2) {
	            		toatString += key + ' harus memiliki 2 uri segment' + "<br>";
	            	}
	            }
	            else if(SubMenu1 != 'Empty' && SubMenu2 != 'Empty')
	            {
