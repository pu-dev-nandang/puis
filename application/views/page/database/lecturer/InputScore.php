...
<script>
    ...
    function getDataScore() {
        ...
        $.post(url,{token:token},function (jsonDataResult) {
            ...
            $('#divScore').html('<h3><b>'+dataMK.MKCode+' - '+dataMK.MKNameEng+'</b></h3>' +
                ...
                '        <hr/>' +
                '<button class="btn btn-warning btn-bg menuDetails" data-page="Schedule_lecturer">Back</button>'+
                '        <button class="btn btn-bg btn-success" id="btnSaveScore">Save</button>'+
                '    </div>');


            for(var g=5;g>=1;g--){
                $('#formTotalAsg').append('<option value="'+g+'">'+g+' Assigment</option>');
            }
            ...
            for(var i=0;i<jsonResult.length;i++){
                ...
                dataIDSturyPlanning.push(dtsc.ID);
            }
        });
