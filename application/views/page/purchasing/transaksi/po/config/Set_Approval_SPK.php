...
<script type="text/javascript">
	...
	function getAllDepartementPU()
	{
	  ...
	  $.post(url,function (data_json) {
	    for (var i = 0; i < data_json.length; i++) {
	    	if (data_json[i]['Code']=='NA.4') {
	    		var selected = (data_json[i]['Code']=='NA.4') ? 'selected' : '';
	...
	function loadPageTable()
	{
		...
		var TableGenerate = '<div class="col-md-12" id = "pageForTable">'+
							...
							;
		TableGenerate += '<tbody>';
		var url = base_url_js+"purchasing/transaction/po/get_cfg_set_roleuser_spk/"+$('#DepartementUserRole').val();
		...
		$(document).off('click', '.btn-delete-setRoleUser').on('click', '.btn-delete-setRoleUser',function(e) {	
			...
			if (ID != '' && ID != null && ID != undefined) {
				...
				if (confirm("Are you sure?") == true) {
				  	...
				  	var url = base_url_js+'purchasing/transaction/po/save_cfg_set_roleuser_spk';
