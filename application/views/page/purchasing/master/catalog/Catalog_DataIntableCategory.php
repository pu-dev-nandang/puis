...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var dataTable = $('#datatablesServer').DataTable( {
		    ...
		    "ordering" : false,
		    "ajax":{
		        url : base_url_js+"purchasing/page/catalog/DataIntableCategory/server_side", // json datasource
		        ordering : false,
		...
		$('#datatablesServer tbody').on('click', '.btn-edit-catalog-category', function () {
			$('.pageAnchor[page="DataIntableCategory"]').trigger('click');
			var ID = $(this).attr('code');
	      	// if (CountColapses == 0) {
	      	if ($('#FormInputCategory').attr('class') == 'collapse') {
	      		//$('.pageAnchor[page="FormInput"]').trigger('click');
	      		//$('#FormInput').show();
	      		$('#FormInputCategory').attr('class','in');
	      		$('#FormInputCategory').attr('style','height: auto;');
	      		var page = 'FormInputCategory';
