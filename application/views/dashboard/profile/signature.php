...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$('body').on('click', '#btnExportCanvas', function(){
			$.fn.bcPaint.export();
		});
		...
		$('body').on('click', '#btnLockCanvas', function(){
			...
			if(isopen == 'close'){
				...
				itsme.find("i.fa").toggleClass('fa-unlock fa-lock');
				itsme.find("span").text("locked");
				var imgData = $.fn.bcPaint.generate();
				$("#Signature-pic").val(imgData);
				$("body #bcPaint").empty().html('<img src="'+imgData+'">');
				itsme.data("arial","show");	
			}else{
				itsme.find("i.fa").toggleClass('fa-unlock fa-spinner').addClass("fa-spin");
                    	...
                    	<div class="row">
	                    	...
	                    	<div class="col-sm-4">
	                    		<div class="well center-block" style="width:100%"> 
		                    		...
		                    		<button type="button" id="btnLockCanvas" class="btn btn-default btn-lg btn-block" data-arial="close"><i class="fa fa-unlock"></i> <span>Unlocked</span></button>
	                    		</div>
	                    	</div>
                    	</div>
                    </div>
