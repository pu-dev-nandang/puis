...
<div class="modal fade" id="modalReqLecturer" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                    ...
                    <div class="data">
                        <div class="row">
                            ...
                            <div class="col-sm-2">
                                ...
                                $("#req-photo").attr("src",base_url_js_server_ws+"uploads/employees/<?=$detail->Photo?>");
                                </script>
...
<script type="text/javascript">
	$(document).ready(function(){
		$("#modalReqLecturer").modal("show");
        <?php if(!empty($detail->Photo)){ ?>
            $(".req-photo").attr("src",url_image +"<?=$detail->Photo?>");
        <?php }else{ ?>
            $(".req-photo").attr("src",window.sessionUrlPhoto);
        <?php } ?>


        $("#modalReqLecturer").on("click",".data-remove",function(){
            var itsme = $(this);
        ...
        /*$("#modalReqLecturer").on("click",".data-resubmit",function(){
            var itsme = $(this);
            var NIP = itsme.data("nip");
            var data = {
                NIP : NIP
        ...
        $("#modalReqLecturer").on("click",".data-resubmit",function(){
            ...
            $(location).attr('href', base_url_js+'profile?resubmit=yes&data='+jwt_encode(data,'UAP)(*'));
        });

        $("#modalReqLecturer").on("click",".data-submit",function(){
            var itsme = $(this);
            if(confirm("Are you sure wants to submit your data to HR ?")){
                ...
                $.ajax({
                    type : 'POST',
                    url : base_url_js+"profile/submit-request",
                    data: {token:token},
                    dataType : 'json',
                    beforeSend :function(){
