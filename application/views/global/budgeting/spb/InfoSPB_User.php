...
<script type="text/javascript">
	...
	function makeDomHTML(se_content)
	{
		__LoadTemplate().then(function(dataTemplate){
			...
			var btnSPb = '';
			var Dis = '';
			var CodeWr = 'Auto Generated by System';
			var LinkFileInvoice = '';
			...
			if (ClassDt.ID_payment !='') {
				...
				GetSupplierChoice = '<label>'+DetailPayment[0].NamaSupplier+'</label>';
				...
				TypeBayar = DetailPayment[0].TypeBayar;
			}	
			html += '<div class = "row"><div class = "col-md-12"><div align="center"><h2>Surat Permohonan Pembayaran</h2></div>'+
						...
						'<tbody>'+
							'<tr>'+
								...
								'</td>'+
								'<td>'+
									'<select class ="form-control SelectTemplate" style = "max-width:300px;"></select>'+
								'</td>'+
							'</tr>'+
							'<tr>'+
								'<td class="TD1">'+
						...
						'<table class="table borderless">'+
							'<thead>'+
								'<tr>'+
									...
									'<td>'+
										'<div class = "col-xs-5">'+
											'<b>'+'<div class = "GetSupplierChoice">'+GetSupplierChoice+'</div>'+'</b>'+
										'</div>'+
										'<div class = "col-xs-5 col-md-offset-1">'+
											OPTypeBayar(TypeBayar,Dis)+
										'</div>'+	
									'</td>'+
								'</tr>'+
	...
	$(document).off('click', '.SearchVendor').on('click', '.SearchVendor',function(e) {
		var ev = $(this);
		var ev_content = ev.closest('#pageContent');
		var html = '';
		html ='<div class = "row">'+
				'<div class = "col-md-12">'+
					'<table id="example_budget" class="table table-bordered display select" cellspacing="0" width="100%">'+
      ...
      '</table></div></div>';
		...
		Get_data_supplier().then(function(data){
			S_Table_example_budget.on( 'click', 'tr', function (e) {
				...
				ev_content.find('.GetSupplierChoice').html(Supplier);
				var aaTemp = ev_content.find('.GetSupplierChoice').find('label').text();
				ev_content.find('.GetSupplierChoice').html(aaTemp);
				$('#GlobalModalLarge').modal('hide');
			} );
		})
	...
	function SubmitData(ID_payment,Action,ID_element)
	{
		...
		// return;
		var TypeBayar = $('.dtTypeBayar option:selected').val();
		var dataInput = {
			CodeSupplier : $('.InpSupplierChoice').val() ,
			NoInvoice : $('.NoInvoice').val(),
			NoTandaTerima : $('.NoTT').val(),
