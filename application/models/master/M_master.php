...
d.`delete`,c.`read` as readMenu,c.`update` as updateMenu,c.`write` as writeMenu,c.`delete` as deleteMenu from db_employees.employees as a
  ...
  public function CountgetNotification()
  {
    $IDDivision = $this->session->userdata('IDdepartementNavigation');
    $sql = "select count(*) as total from db_admission.notification where StatusRead = 0 and ToDiv = ? and ToPeople = 'All' order by Created desc limit 20"; 
  ...
  {
    $IDDivision = $this->session->userdata('IDdepartementNavigation');
    $sql = "select * from db_admission.notification where ToDiv = ? and ToPeople = 'All' order by Created desc limit 20"; 
    $query=$this->db->query($sql, array($IDDivision))->result_array();
    
    return $query;
  }

  public function readNotification($IDDivision)
  {
    $dataSave = array(
               'ReadFirst' => $this->session->userdata('NIP'),
               'StatusRead' => 1,
                       );
    $this->db->where('ToDiv', $IDDivision);
    $this->db->update('db_admission.notification', $dataSave);
  }
}
