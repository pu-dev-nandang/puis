...
//            'LastIPS' => $dataResult[0],
    ...
    public function getDetailStudyPlanning($NPM,$ta){
        $db_ta = 'ta_'.$ta;
        $data = $this->db->query('SELECT s.NPM,s.Name, s.Photo, ma.NIP AS AcademicMentor,s.Gender,
                                    ast.EmailPU ,
        ...
        $dataSemester = $this->_getSemesterActive();
        $smtActID = $dataSemester['ID'];
        $data[0]['DetailSemester'] = $this->getMaxCredit($db_ta,$NPM,$ta,$dataSemester,$smtActID);
    ...
    public function createDBYearAcademicNew($db_new){
        ...
        if ($this->dbforge->create_database($db_new)) {
            ...
            $this->db->query('CREATE TABLE '.$db_new.'.students (
                                  ...
                                  `AcademicMentor` varchar(30) DEFAULT NULL,
            ...
            // study_results
            $this->db->query('CREATE TABLE '.$db_new.'.study_results (
                          `ID` int(11) NOT NULL AUTO_INCREMENT,
                          `NPM` varchar(20) DEFAULT NULL,
                          `SemesterID` int(11) DEFAULT NULL,
                          `CreditSemester` int(11) DEFAULT NULL,
                          `IPS` float DEFAULT NULL,
                          `IPK` float DEFAULT NULL,
                          `TotalSKS` int(11) DEFAULT NULL,
                          `TotalCredit` float DEFAULT NULL,
                          `CreditLulus` int(11) DEFAULT NULL,
                          PRIMARY KEY (`ID`)
...
//        print_r('SELECT ma.* FROM db_academic.mentor_academic ma WHERE '.$Year.' '.$ProdiID.' '.$NIP);
    ...
    public function _checkCourse($ScheduleID){
        ...
        $dataTeamTeaching = $this->db->query('SELECT em.NIP,em.Name AS Lecturer FROM db_academic.schedule_team_teaching stt 
...
//            echo 'SELECT sp.MhswID, s.NPM, s.Name FROM '.$db_.'.study_planning sp
//                                                            LEFT JOIN '.$db_.'.studentd s ON (s.ID = sp.MhswID)
...
//                                                            AND sp.StatusSystem = "1"';
    ...
    public function getScheduleExam($SemesterID,$Type,$ProdiID){
        $data = $this->db->query('SELECT e.*,d.NameEng AS DayEng, cl.Room, em1.Name AS Pengawas1Name, em2.Name AS Pengawas2Name FROM db_academic.exam e 
        ...
        if(count($data)>0){
            for($i=0;$i<count($data);$i++){
                $dataCourse = $this->db->query('SELECT sdc.CDID,mk.MKCode, mk.NameEng AS MKNameEng,em.Name AS Coordinator FROM db_academic.schedule_details_course sdc 
    ...
    public function __getScore($ScheduleID) {
        ...
        $dataCourse = $this->db->query('SELECT sdc.*,mk.NameEng AS MKNameEng, mk.MKCode FROM db_academic.schedule_details_course sdc 
