...
class M_hr extends CI_Model
{
    ...
    public function getSubProdi($NIP)
    {
        $data = $this->db->query('SELECT ps.ID, ps.Name, ps.NameEng, esp.NIP FROM db_academic.program_study ps 
                                        LEFT JOIN db_employees.employees_sub_prodi esp 
                                        ON (esp.ProdiID = ps.ID AND esp.NIP = "' . $NIP . '") 
                                        WHERE ps.Status = 1 ORDER BY ps.NameEng ASC')->result_array();

        $selected = [];
        if (count($data) > 0) {
            for ($i = 0; $i < count($data); $i++) {
                if ($data[$i]['NIP'] == $NIP) {
                    array_push($selected, $data[$i]['ID']);
                }
            }
        }

        $result = array(
            'selectedSubProdi' => $selected,
            'listSubProdi' => $data
        );
