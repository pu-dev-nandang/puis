...
class Form_builder {
    ...
    private $config = array(/* Config array - can be overrided by passing in array in ini() */
        ...
        'default_input_container_class' => 'form-group row',
        'bootstrap_required_input_class' => 'form-control',
        'default_dropdown_class' => 'form-control',
        'default_control_label_class' => 'col-form-label col-md-3',
        'default_no_label_class' => 'col-md-offset-3',
...
//                        print_r($this->elm_options);
//                        echo $this->elm_options['inline'];
                        ...
                        foreach ($all_elm_options['options'] as $elm_suboptions) {
                            ...
                            if ($this->func == 'form_radio') {
                                if ($all_elm_options['value'] == $this->elm_options['value']) {
                                    $this->elm_options['checked'] = 'checked';
                                }
                            } else {
                                if (isset($this->elm_options['checked'])) {
                                    $this->elm_options['checked'] = 'checked';
                                } else {
                                    unset($this->elm_options['checked']);
                                }
                            }
                            ...
                            $label_class = isset($this->elm_options['label_class']) ? $this->elm_options['label_class'] : 'form-check';
                            array_key_exists('inline', $all_elm_options) && $label_class .= ' form-check-inline';
                            array_key_exists('disabled', $this->elm_options) && $label_class .= ' disabled';

                            $this->print_string .= '<div class="' . $label_class . '"><label class="form-check-label">';
                            $this->print_string .= $this->_build_input(FALSE);
                            $this->print_string .= ($this->elm_options['label'] === '' ? '&nbsp;' : $this->elm_options['label']) . '</label></div>'; // Place a nbps to keep the radio button / checkbox aligned with the main label
                        }
        ...
        if ($this->func == 'form_combine') {
            ...
            switch ($this->func) {
                ...
                case 'form_label':
                    ...
                    break;
                case 'form_date':
                    $strpos =  strpos($this->elm_options['class'], 'date');
                    if ($strpos !== false) {
                       $input_html_string = '<div class="input-group input-append date datetimepicker">
                                                <input data-format="yyyy-MM-dd" class="form-control" type=" text" readonly="" value="'.date('Y-m-d').'">
...
//                    if (isset($this->elm_options['options']) && !empty($this->elm_options['options'])) {
                    ...
                    if ($this->func == 'form_multiselect') {
                        $input_html_string = form_multiselect($name, $options, $value, $this->_create_extra_string());
                    } else {
...
//                    }
                ...
                case 'form_textarea':
                    ...
                    #modify by adhi 2020-12-08
                    // print_r($this->elm_options);die();
                    $strpos =  strpos($this->elm_options['class'], 'summernote');
                    if ($strpos !== false) {
                       $input_html_string = '<textarea name="'.$this->elm_options['name'].'" class="form-control area-summernote formTemplateMessage">'.$this->elm_options['value'].'</textarea>';
                    }
                    else
                    {
...
//                        $file_preview = '<img src="https://placeholdit.imgix.net/~text?txtsize=33&txt=Preview&w=200&h=150" alt="...">';
                    ...
                    $input_html_string = '<div class="fileinput ' . $file_input_class . '" data-provides="fileinput">
                            <div class="fileinput-preview fileinput-exists thumbnail img-thumbnail" style="max-width: 200px;">
                                ' . $file_preview . '
                            </div>
                            <div>
                                <span class="btn btn-raised btn-success btn-file">
                                    <span class="fileinput-new"><i class="icon-upload"></i> ' . lang('select_image') . '</span>
                                    <span class="fileinput-exists"><i class="icon-upload"></i> ' . lang('change') . '</span>
                                    <input type="file" name="upload_image" accept="image/*">
                                </span>
                                <input type="hidden" name="delete_image" value="">
                                <a href="#" class="btn btn-raised btn-danger fileinput-exists delete_image" data-dismiss="fileinput"><i class="icon-trash"></i> ' . lang('delete') . '</a>
                            </div>
                        </div>';
                    break;
                default:
        ...
        if ($this->input_addons['exists']) {
            if (!empty($this->input_addons['pre_html'])) {
                ...
                foreach ($this->input_addons['pre'] as $pre_addon) {
                    $ret_string .= '<span class="input-group-prepend">' . $pre_addon . '</span>';
                }
            }
        }
        return $ret_string;
        ...
        if ($this->input_addons['exists']) {
            if (!empty($this->input_addons['post_html'])) {
                ...
                foreach ($this->input_addons['post'] as $post_addon) {
                    $ret_string .= '<span class="input-group-append"><span class="input-group-text">' . $post_addon . '</span></span>';
                }
            }
            $ret_string .= '</div>';
        }
    ...
    private function _pre_elm() {
        $input_container_class = isset($this->elm_options['input_container_class']) ? $this->elm_options['input_container_class'] : $this->config['default_input_container_class'];
        $input_container_id = isset($this->elm_options['input_container_id']) ? 'id="' . $this->elm_options['input_container_id'] . '"' : '';
        return '<div class="' . $input_container_class . '" ' . $input_container_id . '>';
    }
...
//        $this->elm_options['']
        $form_control_class = isset($this->elm_options['form_control_class']) ? $this->elm_options['form_control_class'] : $this->config['default_form_control_class'];
        return '<div class="' . $form_control_class . '">';
    }

    private function _build_help_block() {
        if (!empty($this->elm_options_help)) {
            return '<span class="form-text text-muted">' . $this->elm_options_help . '</span>';
        }
        return '';
    }
